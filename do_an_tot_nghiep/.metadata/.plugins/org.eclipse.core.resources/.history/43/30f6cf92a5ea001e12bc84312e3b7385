...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	if (htim->Instance == TIM3){
		...
		if(count >=10){
//			Convert_int32_to_String4(enc_buff_right, encoder_value_right);
//			Convert_int32_to_String4(enc_buff_left, encoder_value_left);
//			Merge_frame_data_transmit(data, enc_buff_right, enc_buff_left);
//			HAL_UART_Transmit_DMA(&huart2, (uint8_t*)data, 10);

//			sprintf(data_left,"%d",encoder_value_left);
//			HAL_UART_Transmit_DMA(&huart2, (uint8_t*)data_left, 11);
//			sprintf(data_right,"%d",encoder_value_right);
//			HAL_UART_Transmit_DMA(&huart2, (uint8_t*)data_right, 11);
//			for(int i= 0 ; i<22;i++){
//				if(i<11){
//					data[i] = data_left[i];
//				}
